...
import com.example.demo.config.jenkins.HttpClientPool;
import com.example.demo.config.jenkins.HttpUtil;
import com.example.demo.entity.CrumbEntity;
...
public class JenkinsBuildService {
    private String jenkinsHost = "jenkins.fksaas.com";
    private int jenkinsPort = 443;
    private String jenkinsUser = "hushuai";
    private String jenkinsPassword = "hs1234567";
    private CrumbEntity crumbEntity = new CrumbEntity("01e1b9ffa833121144836d0af62d240d","Jenkins-Crumb");
...
//        String format = "https://%s:%s/job/%s/buildWithParameters";
        CrumbEntity crumbEntity = this.getCrumb();
...
//        String format = "http://%s/crumbIssuer/api/json";
        String format = "https://%s:%s/crumbIssuer/api/json";
        CloseableHttpResponse rsp = null;
        HttpGet httpGet = new HttpGet(String.format(format, jenkinsHost, jenkinsPort));
        try {
            System.out.println(this.getHttpClientContext());
